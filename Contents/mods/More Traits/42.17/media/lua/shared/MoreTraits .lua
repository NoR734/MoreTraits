...
internalTick = 0
luckimpact = 1.0
MTModVersion = 42.17 --REMEMBER TO MANUALLY INCREASE
                    ...
                    for i = 0, items:size() - 1 do
                        ...
                        if not processedItems[fullType] then
                            ...
                            if n > 0 then
                                ...
                                not isServer()
                                        and MT_Config
                                        and MT_Config:getOption("ScroungerAnnounce"):getValue()
                                then
                                    HaloTextHelper.addTextWithArrow(
                                            player,
                                            getText("UI_trait_scrounger") .. ": " .. item:getName(),
                                            true,
                                            HaloTextHelper.getColorGreen()
                                    )
                                end
                            ...
                            if not processedItems[fullType] then
                                ...
                                if removeCount > 0 then
                                    ...
                                    then
                                        HaloTextHelper.addTextWithArrow(
                                                player,
                                                getText("UI_trait_incomprehensive") .. " : " .. item:getName(),
                                                false,
                                                HaloTextHelper.getColorRed()
                                        )
                                    end
                    ...
                    for i = 1, iterations do
                        if ZombRand(100) <= baseChance then
                            ...
                            if itemName and MT_Config and MT_Config:getOption("VagabondAnnounce"):getValue() then
                                HaloTextHelper.addTextWithArrow(
                                        player,
                                        getText("UI_trait_vagabond") .. " : " .. itemName,
                                        true,
                                        HaloTextHelper.getColorGreen()
                                )
                            end
...
local function SpecializationAndAntiGun(player, perk, amount)
    if skipxpadd or amount <= 0 then
        return
    end

    local hasAnySpec = false
    if
    player:hasTrait(ToadTraitsRegistries.specweapons)
            or player:hasTrait(ToadTraitsRegistries.specfood)
            or player:hasTrait(ToadTraitsRegistries.specguns)
            or player:hasTrait(ToadTraitsRegistries.specmove)
            or player:hasTrait(ToadTraitsRegistries.speccrafting)
            or player:hasTrait(ToadTraitsRegistries.specaid)
    then
        hasAnySpec = true
    end
    ...
    if hasAnySpec then
        ...
        -- Full XP for those who are specialised.
    ...
    if totalPenaltyMultiplier > 0 then
        -- This is mainly to make sure if they pick "AntiGun" as a perk they still get XP, even if it's a very very small amount.
        ...
        if isClient() then
            if math.abs(newPain - playerdata.fBadTeethLastSentPain) >= 1 then
                ...
                sendClientCommand(
                        ...
                        "ToadTraits",
                        "BodyPartMechanics",
                        { bodyPart = headPart, partPain = newPain }
                )
    ...
    if isBareHands and allow then
        ...
        if not isServer() and MT_Config and MT_Config:getOption("MartialDamage"):getValue() then
            HaloTextHelper.addText(
                    player,
                    "Damage: " .. tostring(round(martialDamage, 3)),
                    " ",
                    HaloTextHelper.getColorGreen()
            )
        end
...
local function MT_MeleeTraits(actor, target, weapon, damage)
    ...
    if player:hasTrait(ToadTraitsRegistries.tavernbrawler) then
        ...
        weapon:isOfWeaponCategory(WeaponCategory.IMPROVISED)
                or tableContains(whitelist, weapon:getDisplayCategory() or "")
        then
            isImprovised = true
        end
...
local function amputee(player, justGotInfected)
    ...
    if needToHeal or justGotInfected then
        if isClient() then
            local args = { zombie_fever = 0, zombie_infection = 0, clear_wounds = true, amputee = true }
            sendClientCommand(player, "ToadTraits", "UpdateStats", args)
        else
...
local function MT_FastGimpMove(player)
    ...
    local isWalking = player.isWalking and player:isWalking() or false

    -- Build 42 updates occasionally desync movement-state flags in MP.
    -- If we are clearly moving but none of the state methods reports true,
    -- treat it as walking so Fast/Gimp still applies at least the baseline modifier.
    if not isSprinting and not isRunning and not isWalking and player:isPlayerMoving() then
        isWalking = true
    end
        ...
        if isClient() then
            for _, data in ipairs(anemicParts) do
                sendClientCommand(
                        ...
                        "ToadTraits",
                        "BodyPartMechanics",
                        { bodyPart = data.part, partHealthReduce = data.amount }
                )
            end
        else
            for _, data in ipairs(anemicParts) do
        ...
        if isClient() then
            for _, data in ipairs(thickParts) do
                sendClientCommand(
                        ...
                        "ToadTraits",
                        "BodyPartMechanics",
                        { bodyPart = data.part, partHealthAdd = data.amount }
                )
            end
        else
            for _, data in ipairs(thickParts) do
...
local function ClearZombieInfection(player, stats, bodyDamage, setFever)
    if isClient() then
        local args = { zombie_infection = 0, clear_wounds = true }
        if setFever then
            args.zombie_fever = 100
        end
        sendClientCommand(player, "ToadTraits", "UpdateStats", args)
        return
    end
    ...
    if setFever then
        -- Set Fever once when Super Immune first catches an infection,
        -- then let the recovery loop drive fever down naturally.
...
local function SuperImmune(player, playerdata)
    ...
    if playerdata.SuperImmuneActive then
        if zombieInfection > 0 or hasBodyInfection then
            -- Do not reset fever/recovery while already recovering.
            ClearZombieInfection(player, stats, bodyDamage, false)
        end
        return
    end
    ...
    ClearZombieInfection(player, stats, bodyDamage, true)
...
local function SuperImmuneRecoveryProcess(player, playerdata)
    ...
    if timeElapsed < maxRecoveryMinutes then
        ...
        if illness > 0 then
            if
            not playerdata.SuperImmuneFeverNotified
                    and not isServer()
                    and MT_Config
                    and MT_Config:getOption("SuperImmuneAnnounce"):getValue()
            then
                HaloTextHelper.addTextWithArrow(
                        player,
                        getText("UI_trait_superimmune_feverbreak"),
                        true,
                        HaloTextHelper.getColorGreen()
                )
        ...
        if
        illness == 0
                and not playerdata.SuperImmuneTextSaid
                and not isServer()
                and MT_Config
                and MT_Config:getOption("SuperImmuneAnnounce"):getValue()
        then
            HaloTextHelper.addTextWithArrow(
                    player,
                    getText("UI_trait_superimmunewon"),
                    true,
                    HaloTextHelper.getColorGreen()
            )
...
local function Immunocompromised(player)
    ...
    if not bodyDamage:HasInjury() then
        return
    end -- Early exit if there is no actual injuries
                ...
                if not modData.bGraveRobberRolled then
                    ...
                    if ZombRand(0, 1001) <= chance then
                        ...
                        if #itemsFound > 0 then
                            ...
                            not isServer()
                                    and MT_Config
                                    and MT_Config:getOption("GraveRobberAnnounce"):getValue()
                            then
                                HaloTextHelper.addTextWithArrow(
                                        player,
                                        getText("UI_trait_graverobber"),
                                        true,
                                        HaloTextHelper.getColorGreen()
                                )
                            end
                            ...
                            if isClient() then
                                local args = {
                                    x = math.floor(containerObj:getX()),
                                    y = math.floor(containerObj:getY()),
                                    z = math.floor(containerObj:getZ()),
                                    items = itemsFound,
                                }
            ...
            for i = 0, enemies:size() - 1 do
                ...
                if enemy and enemy:isZombie() then
                    ...
                    if distance <= 1.0 and canBeHit and not enemy:isKnockedDown() then
                        ...
                        if
                        player:hasTrait(ToadTraitsRegistries.martial)
